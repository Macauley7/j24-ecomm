...
export const EcomProvider = ({ children }) => {
    ...
    const fetchProduct = async () => {
        try{
            const response = await fetch("https://j24-ecomm-api.onrender.com/api/products", {
                method: "GET",
                headers: {
    ...
    if (isAuthenticated) {
      // if authenticated
      try {
        const res = await fetch("https://j24-ecomm-api.onrender.com/api/add-to-cart", {
          method: "POST",
          headers: {
   ...
   const fetchCart = async () => {
    if (isAuthenticated) {
      // authenticated
      const res = await fetch("https://j24-ecomm-api.onrender.com/api/carts", {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          "auth-token": `${localStorage.getItem("auth-token")}`,
  ...
  const removeCartItems = async (productId) => {
    if (window.confirm("Are you sure you want to delete?..")) {
      if (isAuthenticated) {
        try {
          // authenticated
          const res = await fetch("https://j24-ecomm-api.onrender.com/api/delete-cart-items", {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
              "auth-token": `${localStorage.getItem("auth-token")}`,
    ...
    const updateCartItem = async (productId, quantity) => {
      ...
      if (isAuthenticated) {
        try {
          const res = await fetch("https://j24-ecomm-api.onrender.com/api/update-cart-items", {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
              "auth-token": `${localStorage.getItem("auth-token")}`,
    ...
    const createOrder = async (transaction_Id, orderId) => {
      try {
        const response = await fetch(
          "https://j24-ecomm-api.onrender.com/api/verify-payment",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
